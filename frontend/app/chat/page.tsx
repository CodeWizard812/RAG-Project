...
export default function ChatPage() {
  ...
  async function switchSession(id: string) {
    setActiveSessionId(id);
    setMessages([]);   // load history here if needed
  }
      ...
      for await (const event of stream) {
        ...
        if (type === "tool_start") {
          ...
          setMessages(prev => prev.map(m => {
            ...
            const idx    = active.indexOf(active[active.length - 1] ?? "");
            return {
              ...m,
              activeTools: active.slice(0, idx),
              toolCalls: [
                ...(m.toolCalls ?? []),
                {
                  tool:  active[active.length - 1] ?? "unknown",
                  input: {},
                },
              ],
            };
          ...
          // Animate answer word-by-word for visual effect
          const answer  = event.answer as string;
          const sources = parseContextsToSources(
            (event.contexts as string[]) ?? []
          );
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="bg-white border-b border-gray-100 px-6 py-3 flex items-center justify-between">
          <div className="text-sm text-gray-500">
            Session: <span className="font-medium text-gray-800">{activeSessionId}</span>
          </div>
